...
public class EthernetStateTracker extends Handler implements NetworkStateTracker {
    ...
    if (localLOGV) Slog.d(TAG, "configureInterface " + info.getIfName());
		if(mNfsmode && "eth0".equals(info.getIfName()))
		{
            if (localLOGV) Slog.i(TAG, "nfs return configureInterface " + info.getIfName());
			return true;
		}
        ...
        if (info.getNetMask() != null) {
            try {
                InetAddress inetAddress = NetworkUtils.numericToInetAddress(info.getNetMask());
                if (inetAddress instanceof Inet4Address) {
                    mDhcpInfo.prefixLength = NetworkUtils.netmaskIntToPrefixLength( 
							NetworkUtils.inetAddressToInt((Inet4Address)inetAddress));
                }
            } catch (IllegalArgumentException e) {}
        }
            Slog.v(TAG, "Static prefixLength = " + mDhcpInfo.prefixLength);
                 ...
                 case EVENT_DHCP_START:
                     synchronized (mDhcpTarget) {
                         if (!mInterfaceStopped) {
                             ...
                             if (NetworkUtils.runDhcp(mInterfaceName, new android.net.DhcpResults(mDhcpInfo.makeLinkProperties()))) {
                                 event = EVENT_INTERFACE_CONFIGURATION_SUCCEEDED;
...
//        mEM.captivePortalCheckComplete();
    }
