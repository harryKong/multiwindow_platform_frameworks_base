...
namespace android {
    ...
    static jstring android_net_ethernet_waitForEvent(JNIEnv *env, jobject clazz)
    {
        ...
        ALOGE("Poll events from ethernet devices");
        /*
        ...
        if ((len = recvmsg(nl_socket_poll, &msg, 0)) >= 0) {
            ...
            for (nh = (struct nlmsghdr *) buff; NLMSG_OK (nh, len);
                ...
                ALOGE(" event :%d  found", nh->nlmsg_type);
                einfo = (struct ifinfomsg *)NLMSG_DATA(nh);
                ALOGE("the device flag :%X", einfo->ifi_flags);
            ...
            ALOGE("Done parsing");
            rbuf[4096 - left] = '\0';
            ALOGE("poll state :%s, left:%d", rbuf, left);
        }
        ...
        if ((netdir = opendir(SYSFS_CLASS_NET)) != NULL) {
             while ((de = readdir(netdir))) {
                if ((!strcmp(de->d_name, ".")) || (!strcmp(de->d_name, ".."))
                    ...
                    (!strcmp(de->d_name, "pan0")) || (!strcmp(de->d_name, "ip6tnl0"))
                    || (!strcmp(de->d_name, "can0")) || (!strcmp(de->d_name, "can1"))
                                                         ...
                                                         jint index)
    {
        int i = 0;
        interface_info_t *info;
        ALOGE("User ask for device name on %d, list:%X, total:%d",
             index, (unsigned int)interfaces, total_int);
        info = interfaces;
        if (total_int != 0 && index <= (total_int - 1)) {
            while (info) {
                if (index == i) {
                    ALOGE("Found: %s", info->name);
